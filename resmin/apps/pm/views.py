...
if "notification" in settings.INSTALLED_APPS and getattr(settings, 'apps.pm_NOTIFY', True):
...
@login_required
def inbox(request, template_name='pm/inbox.html'):
    """
...
@login_required
def outbox(request, template_name='pm/outbox.html'):
    """
...
@login_required
def trash(request, template_name='pm/trash.html'):
    """
...
def delete(request, message_id, success_url=None):
    ...
    different page (e.g. `/foo/bar/`) than ``success_url`` after deletion
    of the message.
    """
    user = request.user
    now = timezone.now()
